...
SteadyVectorField2D VastistasVelocityGenerator::generateSteadyMixtureOFVortexBoundaryPaper(const std::vector<VastisParamter>& vectorFieldMeta) const noexcept
{
	std::vector<std::vector<Eigen::Vector2d>> data_(mgridDim_y, std::vector<Eigen::Vector2d>(mgridDim_x, Eigen::Vector2d{ 0.0, 0.0 }));
	const int mix = vectorFieldMeta.size();
	std::vector <std::function<Eigen::Vector2d(const Eigen::Vector2d& pos, double t) >>	fieldFunctions;
	fieldFunctions.reserve(mix);

	for (size_t i = 0; i < mix; i++) {
		const  auto& parm = vectorFieldMeta.at(i);
		...
		const double rc = rc_n.x();
		const double n = rc_n.y();
		...
		const auto SiMat22 = SiMatices_[static_cast<int>(si)];
		auto lambdaFuncThisField = [this, SiMat22, rc, n, txy, deformMatA](const Eigen::Vector2d& pos, double t) -> Eigen::Vector2d {
			auto originalPos = deformMatA.inverse() * (pos - txy);
			auto standardVastistasVelocity = SiMat22 * originalPos * NormalizedVastistasV0_Fn(originalPos.norm(), rc, n);
			Eigen::Vector2d  transformedVastistasVelocity = deformMatA * standardVastistasVelocity;
			return transformedVastistasVelocity;
			};
		fieldFunctions.emplace_back(lambdaFuncThisField);
	}

	auto lambdaFunc = [this, fieldFunctions, mix](const Eigen::Vector2d& pos, double t) -> Eigen::Vector2d {
		Eigen::Vector2d result = Eigen::Vector2d::Zero();
		for (int i = 0; i < mix; i++) {
			result += fieldFunctions[i](pos, t);
		}
		return result;
		};
	...
	steadyField.analyticalFlowfunc_ = lambdaFunc;
	steadyField.resampleFromAnalyticalExpression();
	return steadyField;
}
