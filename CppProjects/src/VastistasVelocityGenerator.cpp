...
velocityFieldData VastistasVelocityGenerator::generate() const noexcept
{
    return generate(1.0, 1.0, 0.0, 1);
}

// resample VastistasVelocity to discrete grid
velocityFieldData VastistasVelocityGenerator::generate(float sx, float sy, float theta, int Si) const noexcept
{
    ...
    for (size_t i = 0; i < mgridDim_y; i++)
        for (size_t j = 0; j < mgridDim_x; j++) {
            ...
            const float v0 = NormalizedVastistasV0(xy);
