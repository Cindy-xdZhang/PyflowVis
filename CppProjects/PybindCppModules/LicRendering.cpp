...
py::array_t<double> licRenderingPybindCPPv2(
	py::array_t<float> vector_field_np,
	int Xdim,
	int Ydim,
	double xMin,
	...
	int MaxIntegrationSteps)
{
	...
	if (YdimData == Ydim && XdimData == Xdim && vector_field_info.shape[2] == 2) {
		...
		// Call LICAlgorithm output texture[y][x]
		std::vector<std::vector<Eigen::Vector3d>> result = LICAlgorithm(vecfield, licImageSizeX, licImageSizeY, stepSize, MaxIntegrationSteps, static_cast<VORTEX_CRITERION>(0));

		// Convert result to numpy array
		py::array_t<double> result_np({ licImageSizeY, licImageSizeX, 3 }); // 3 for R, G, B
		py::buffer_info result_info = result_np.request();
		double* result_ptr = static_cast<double*>(result_info.ptr);
		for (int y = 0; y < licImageSizeY; ++y) {
			for (int x = 0; x < licImageSizeX; ++x) {
				for (int c = 0; c < 3; ++c) {
					result_ptr[(y * licImageSizeX + x) * 3 + c] = result[y][x](c);
				}
			}
		}
...
PYBIND11_MODULE(CppLicRenderingModule, m)
{
	...
	m.attr("__version__") = "0.0.4-stable_noise_lic";
    ...
    int MaxIntegrationSteps\
        ");
	m.def("licRenderingPybindCPPv2", &licRenderingPybindCPPv2, "Render a steady vector field using LIC.\
     @params:\
    py::array_t<float> vector_field_np->feed with numpy 3d tensor(xdim,ydim,vectorComponnet=2),\
    int Xdim,\
    int Ydim,\
