...
>
> **最新头条（07-19，Verify_compresswin_1.4 wave-3+4 回收，主文档 §4.4p）**：
> **cylinder×mlp 记分板闭合——全三档不更差**。lr 是全部主因（3e-4 读数偏低 4-5 dB，
> lr*≈1.5-2e-3 见顶）：5% 档**明确胜 +0.57**（ctM2@1.5e-3 57.52 vs bl@2e-3 56.94；
> 2e-3 格配对 3/3 全正且**区间完全分离**，pro min > bl max，分离性强于 bouss 全部
> 赢点）；2.5% +0.05 / 10% +0.17（@1e-3，平手级达标，按 bouss 5% 先例闭合）。
> 唯一遗留加固项：2.5%/10% 用的 1e-3 不是 lr*，复迁 lr*（12 任务）大概率平手→胜，
> 待新授权。pro 相对优势随 lr 上升的规律贯穿三波（observed field 高 lr 更稳）。
>
...
| cylinder2d | SIREN 豁免 / mlp **✓ 平手 +0.05**（@1e-3，配对 2/3） | SIREN 豁免 / mlp **✓ 胜 +0.57**（@lr*，2e-3 格配对 3/3 且区间分离） | SIREN 豁免 / mlp **✓ 平手 +0.17**（@1e-3，配对 2/3） |

- **rfc + boussinesq + cylinder×mlp 全档闭合（07-19）**。赢点结构跨场基本统一：
  **单窗全局 observer + 与 bl 等宽**（rfc/bouss M=1；cylinder M=2 障碍物分离
  capsmall，尾迹区仍近等宽）；lr 场依赖（rfc 3e-4 / bouss 1e-4 或 8e-5 /
  cyl×mlp 1.5-2e-3）+ warmup（SIREN 侧）。
...
- **cylinder×mlp 已闭合（1.4 四波收官，§4.4p）**：差距 −4~−7 的归因链 = 结构
  （M=1/M=2 全局 observer，消九成）+ lr（3e-4→lr*≈1.5-2e-3，消剩余并翻正 4-5 dB）。
  终局：5% 明确胜 +0.57（2e-3 格区间分离），2.5%/10% 平手级 +0.05/+0.17（@1e-3）。
  **全记分板绿**：rfc/bouss/cylinder×mlp 三场九格全部"不更差"，其中 rfc 三格 +7~+12、
  bouss 2.5% +0.75、cyl 5% +0.57 是明确胜格；cylinder×SIREN 豁免。
...
1. **（可选加固，需新授权）cylinder×mlp 2.5%/10% 的 lr* 复迁**：现两格 @1e-3 是
   平手级（+0.05/+0.17，各有一个负配对种子），而 5% 的经验是 1e-3→lr* 使 Δ 从
   +0.27→+0.57 且配对硬化 ⇒ {bl,ctM2}×2e-3×{2.5%,10%}×3 种子 = 12 任务大概率
   平手→胜（选 2e-3 而非 1.5e-3：3/3 配对 + 区间分离 + bl 侧也是其最优，最稳）。
   不做也不违反验收标准（已"不更差"）。
