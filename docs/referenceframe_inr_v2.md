...
**Wave-4（用户 2026-07-19 裁定"A 和 B 都部署"= 24 任务；Ibex job 49121631，
脚本 `refframe_v2_compresswin4d.sh`，commit 346b0b5，结果待回收）**：
- **A 段（任务 0-11）**：{bl, ctM2-capsmall} × lr 1e-3 × {2.5%, 10%} × 3 种子——把
  wave-3 的赢点 lr 迁到仅剩的两个未达标格；两臂同 lr 重跑（旧读数 @3e-4 不可比），
  最优对最优口径不变。
- **B 段（任务 12-23）**：{bl, ctM2-capsmall} × lr {1.5e-3, 2e-3} × 3 种子 @5%——
  续探未见顶的 lr 阶梯（2e-3 可能触 mlp 稳定边界，grad_clip=1.0 在）。
- **冒烟（本地，1 epoch）**：ctM2-capsmall 新档 sizing 全过——2.5%：m=19/8、
  639,949 B ≤ 655,360（CR=41×）；10%：m=41/8（尾迹与 bl m=42 近等宽）、
  2,556,853 B ≤ 2,621,440；M=2 边信息 12,885 B（含 N=2 标签图，口径 v2 内）。
  bl 两档与 B 段结构均为已验证配置仅改 lr，免烟。

**Wave-4 结果（2026-07-19 回收，24/24 零失败；3-seed 均值 [min..max]，出处 job
49121631）**：

A 段（lr=1e-3 迁移，仅剩两格）：

| frac | bl | ctM2-capsmall | Δ均值 | 种子配对（s0/s7777/s1） |
|---|---|---|---|---|
| 2.5% | 54.53 [53.84..55.08] | 54.58 [53.98..55.10] | **+0.05** | +0.83 / +0.43 / −1.10（2/3 正） |
| 10% | 57.90 [57.72..58.01] | 58.06 [57.76..58.24] | **+0.17** | −0.25 / +0.48 / +0.28（2/3 正） |

B 段（5% lr 阶梯延伸；wave-3 的 1e-3 行并列供读）：

| lr @5% | bl | ctM2-capsmall | Δ均值 | 种子配对 |
|---|---|---|---|---|
| 1e-3（wave-3） | 56.46 [56.10..56.75] | 56.73 [56.58..56.82] | +0.27 | 3/3 正 |
| 1.5e-3 | 56.79 [56.36..57.62] | **57.52 [57.21..57.82]** | **+0.72** | +1.43 / −0.11 / +0.85（2/3 正） |
| 2e-3 | 56.94 [56.76..57.06] | 57.51 [57.40..57.73] | **+0.57** | +0.35 / +0.38 / +0.97（**3/3 正，区间完全分离**：pro min 57.40 > bl max 57.06） |

**读数**：
1. **5% 档 lr 阶梯见顶，胜幅扩大并硬化**：ctM2 在 1.5e-3（57.52）与 2e-3（57.51）
   持平 ⇒ lr*≈1.5-2e-3；bl 边际增幅也降到 +0.15。最优对最优 = ctM2@1.5e-3 57.52 vs
   bl@2e-3 56.94 = **+0.57**；最干净的一格是 2e-3：配对 3/3 全正且**区间分离**
   （pro 最差种子 > bl 最好种子 +0.34）——分离性强于 bouss 全部赢点。
   pro 相对优势随 lr 继续上行（+0.27 → +0.72/+0.57），2e-3 下 mlp 两臂均无坍缩。
2. **2.5%/10% @1e-3 翻到非负**：−0.86→+0.05、−0.61→+0.17（均值微正、配对 2/3、
   区间大幅重叠）——按 bouss 5% 先例（+0.002 判"不更差"闭合）两格达标，但成色是
   **平手级**，各有一个负配对种子（2.5% s1 −1.10、10% s0 −0.25）。
3. **A 段用的 1e-3 不是 lr***（部署时 B 段结果未知）：5% 的经验是 1e-3→lr* 使 Δ 从
   +0.27 → +0.57~0.72 且配对硬化 ⇒ 2.5%/10% 若用 lr* 复迁（12 任务：两臂 ×
   {1.5e-3 或 2e-3}× 两档 ×3 种子）大概率把平手推成明确胜——待新授权，是唯一遗留
   加固项。
4. lr 效应跨档一致：bl@2.5% 从 3e-4 的 49.80 → 1e-3 的 54.53（+4.7）；bl@10%
   54.91 → 57.90（+3.0）。§4.4l "lr 是压缩口径主杠杆"在 mlp 上完全成立。

**Verify_compresswin_1.4 判定第三版（新旧并列）**：wave-2 后"5% 平手、2.5/10% 差
0.6-0.9，未闭合"→ wave-3 后"5% 弱胜 +0.27，2.5/10% 未复测"→ **现在（wave-4 后）：
"cylinder×mlp 全三档不更差——5% 明确胜 +0.57（配对 3/3、区间分离），2.5% +0.05 /
10% +0.17 平手级达标"**。变化原因 = lr 阶梯探到位（3e-4 → lr*≈1.5-2e-3 = +4~5 dB）。
按验收标准（proposed 不得比对应架构 baseline 更差）**cylinder×mlp 记分板闭合**；
2.5%/10% 的成色升级（平手→胜）留作 lr* 复迁可选项。
